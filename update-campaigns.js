...
async function updateCampaigns() {
  try {
    ...
    const Commerce = mongoose.model('Commerce', new mongoose.Schema({
      slug: String
    ...
    for (const campaign of campaigns) {
      if (campaign.qrCodeUrl && campaign.qrCodeUrl.includes('/lottery?')) {
        ...
        campaign.qrCodeUrl = campaign.qrCodeUrl.replace('/lottery?', '/welcome?');
        await campaign.save();
        ...
        if (commerce) {
          ...
          await campaign.save();
          console.log(`✅ Generated URL for campaign ${campaign._id}: ${campaign.qrCodeUrl}`);
        }
      }
    }
