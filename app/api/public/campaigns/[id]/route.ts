...
export async function GET(req: NextRequest, context: RouteContext) {
  try {
    ...
    console.log('[PUBLIC API] Campaign data:', campaign ? JSON.stringify({
      _id: campaign._id,
      name: campaign.name,
    ...
    if (campaign.commerceId) {
      ...
      commerceData = await Commerce.findById(campaign.commerceId)
        .select('name slug googleBusinessUrl')
        .lean();
    ...
    if (campaign.prizePoolId) {
      ...
      try {
        ...
        if (prizePool && prizePool.prizes && Array.isArray(prizePool.prizes)) {
          ...
          const prizesList = await Prize.find({ _id: { $in: prizeIds } }).lean();
          ...
          prizes = prizesList.map((prize: any) => {
            const poolPrize = prizePool.prizes.find((p: any) =>
              p.prizeId.toString() === prize._id.toString()
            );
            return {
              ...prize,
              probability: poolPrize?.probability || { mode: 'fixed', fixedPercent: 0 },
            };
          });
