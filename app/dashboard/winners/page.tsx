...
export default function WinnersPage() {
  ...
  // Filtrage en temps réel lors de la recherche par code
  useEffect(() => {
    if (searchCode.trim()) {
      searchByCodeRealtime();
    } else {
      fetchWinners();
    }
  }, [searchCode]);
  ...
  const searchByCodeRealtime = async () => {
    if (!searchCode.trim()) {
      fetchWinners();
      return;
    }
    ...
    try {
      // Envoyer directement ce que l'utilisateur tape (sans forcer RVW-)
      // L'API ajoutera le préfixe et fera une recherche "starts with"
      const response = await fetch(`/api/winners?code=${encodeURIComponent(searchCode.toUpperCase())}`);
      const data = await response.json();
      setWinners(data.winners || []);
    } catch (error) {
          ...
          <div className="flex-1">
            ...
            <div className="relative">
              <input
                ...
                placeholder="Ex: M ou MASV5B"
                className="w-full px-4 py-2 pl-10 pr-32 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent uppercase"
              />
              <Search className="w-5 h-5 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2" />
              {searchCode && (
                <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs font-mono text-blue-600 bg-blue-50 px-2 py-1 rounded border border-blue-200">
                  RVW-{searchCode.toUpperCase()}
                </div>
              )}
            </div>
          </div>
