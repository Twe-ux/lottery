    ...
    callbacks: {
      authorized: ({ token, req }) => {
        ...
        if (path.startsWith('/dashboard')) {
          ...
          const isAdmin = role ? ['super_admin', 'admin', 'commerce_admin', 'employee'].includes(role) : false;
          const isAuthorized = hasToken && isAdmin;
          console.log('🔐 [MIDDLEWARE] Dashboard - Token:', hasToken, '- Rôle:', role, '- Autorisé:', isAuthorized);
          return isAuthorized;
        }
